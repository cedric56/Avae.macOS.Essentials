	...
	class MapImplementation : IMap
	{
		...
		public async Task<bool> TryOpenAsync(Placemark placemark, MapLaunchOptions options)
		{
			...
			var address = new NSMutableDictionary
			{
				...
				["Country"] = new NSString(placemark.CountryName ?? string.Empty),
				["State"] = new NSString(placemark.AdminArea ?? string.Empty),
				["Street"] = new NSString(placemark.Thoroughfare ?? string.Empty),
